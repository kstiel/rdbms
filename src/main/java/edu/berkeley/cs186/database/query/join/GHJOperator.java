...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        // to get a hash value.
        if (left) {
            for (Record lRecord: records) {
                DataBox colValue = lRecord.getValue(getLeftColumnIndex());
                int hash = HashFunc.hashDataBox(colValue, pass);
                int index = hash % partitions.length;
                if (index < 0 ) {
                    index += partitions.length;
                }
                partitions[index].add(lRecord);
            }
        }
        else {
            for (Record rRecord: records ) {
                DataBox colValue = rRecord.getValue(getRightColumnIndex());
                int hash = HashFunc.hashDataBox(colValue, pass);
                int index = hash % partitions.length;
                if (index < 0 ) {
                    index += partitions.length;
                }
                partitions[index].add(rRecord);
            }
        }
    }
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for ( Record record: buildRecords) {
            DataBox leftJoinValue = record.getValue(buildColumnIndex);
            if (!hashTable.containsKey(leftJoinValue)) {
                hashTable.put(leftJoinValue, new ArrayList<>());
            }
            hashTable.get(leftJoinValue).add(record);
        }

        for ( Record record: probeRecords ) {
            DataBox rightJoinValue = record.getValue(probeColumnIndex);
            if (!hashTable.containsKey(rightJoinValue)) continue;
            for ( Record current : hashTable.get(rightJoinValue) ) {
                Record joinedRecord = current.concat(record);
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            int leftLen = leftPartitions[i].getNumPages();
            int rightLen = rightPartitions[i].getNumPages();
            if ( leftLen <= numBuffers - 2 ){
                buildAndProbe(leftPartitions[i], rightPartitions[i]);
            }
            else if ( rightLen <= numBuffers - 2 ){
